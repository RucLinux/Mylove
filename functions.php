...
/**
 * 评论等经 wp_kses_data（context=data）时默认不含 img，主题插入的表情图会被剥掉。
 */
function xghome_classic_kses_allow_emoji_img_tags($tags, $context)
{
    ...
    $ctx = (string) $context;
    if (!in_array($ctx, ['data', 'comment'], true)) {
        return $tags;
    }
    $tags['img'] = [
        'src'      => true,
...
add_filter('wp_kses_allowed_html', 'xghome_classic_kses_allow_emoji_img_tags', 10, 2);
