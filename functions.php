...
/**
 * 多行 <code> 在 theme.css 中加 .xghome-code-multiline，与已有 class 合并。
 *
 * @param string $attrs `<code` 后的属性串（含前导空格，或为空）
 */
function xghome_classic_code_attrs_add_multiline_class(string $attrs, bool $multiline): string
{
    if (!$multiline) {
        return $attrs;
    }
    $extra = 'xghome-code-multiline';
    if (preg_match('/\bclass\s*=\s*"([^"]*)"/', $attrs, $cm)) {
        $merged = trim($cm[1] . ' ' . $extra);
        return preg_replace('/\bclass\s*=\s*"[^"]*"/', 'class="' . esc_attr($merged) . '"', $attrs, 1);
    }
    if (preg_match("/\bclass\s*=\s*'([^']*)'/", $attrs, $cm)) {
        $merged = trim($cm[1] . ' ' . $extra);
        return preg_replace("/\bclass\s*=\s*'[^']*'/", "class='" . esc_attr($merged) . "'", $attrs, 1);
    ...
    $t = trim($attrs);
    if ($t === '') {
        return ' class="' . esc_attr($extra) . '"';
    }
    return ' ' . $t . ' class="' . esc_attr($extra) . '"';
}

/**
 * wpautop 会在 <code> 内插入过多 <br />，行距过大。将换行还原为普通换行符（theme.css 中 code 已 white-space: pre-wrap）。
 * 多行时加 class，使块级展示、背景连成一片（行内 code 换行会每行一块背景）。
 */
function xghome_classic_normalize_code_line_breaks(string $content): string
{
    return (string) preg_replace_callback(
        ...
        static function (array $m): string {
            ...
            $inner = preg_replace("/\n{3,}/", "\n\n", $inner);
            $inner = trim($inner);
            $attrs = xghome_classic_code_attrs_add_multiline_class($attrs, strpos($inner, "\n") !== false);
            return '<code' . $attrs . '>' . $inner . '</code>';
        },
        $content
    );
}
add_filter('the_content', 'xghome_classic_normalize_code_line_breaks', 12, 1);
...
function xghome_classic_get_permalink_options(): array
{
    return [
        ...
        'watermark_text_px' => (int) get_option('xghome_watermark_text_px', 24),
        'watermark_skip_max_px' => (int) get_option('xghome_watermark_skip_max_px', 64),
        ...
        'ip_geo_enabled' => (int) get_option('xghome_ip_geo_enabled', 0),
        'ip_geo_data_dir' => (string) get_option('xghome_ip_geo_data_dir', ''),
        'ip_geo_trust_cf' => (int) get_option('xghome_ip_geo_trust_cf', 0),
        'ip_geo_api_config' => (string) get_option('xghome_ip_geo_api_config', ''),
        'ip_geo_baidu_fallback' => (int) get_option('xghome_ip_geo_baidu_fallback', 1),
    ];
}
...
function xghome_classic_add_theme_menu(): void
{
    ...
    add_theme_page(
        __('网站地图', 'xghome-classic'),
        __('网站地图', 'xghome-classic'),
        'manage_options',
        'xghome-sitemap',
        'xghome_classic_render_sitemap_admin_page'
    );
}
add_action('admin_menu', 'xghome_classic_add_theme_menu');

require_once get_template_directory() . '/inc/sitemap-admin.php';

define('XGHOME_IP_GEO_VARIANT', 'full');
require_once get_template_directory() . '/inc/xghome-ip-geo.php';
require_once get_template_directory() . '/inc/xghome-walker-comment.php';
...
function xghome_classic_handle_permalink_save(): void
{
    ...
    $watermark_skip_max_px = isset($_POST['xghome_watermark_skip_max_px'])
        ? (int) sanitize_text_field(wp_unslash($_POST['xghome_watermark_skip_max_px']))
        : (int) $saved_options['watermark_skip_max_px'];
    $watermark_text_px = isset($_POST['xghome_watermark_text_px'])
        ? (int) sanitize_text_field(wp_unslash($_POST['xghome_watermark_text_px']))
        : (int) $saved_options['watermark_text_px'];
    ...
    update_option('xghome_watermark_skip_max_px', max(16, min(512, $watermark_skip_max_px)));
    update_option('xghome_watermark_text_px', max(12, min(72, $watermark_text_px)));
    ...
    xghome_ip_geo_handle_theme_save($saved_options);
            ...
            <table class="form-table" role="presentation">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <p><label for="xghome_watermark_text_px">文字水印字号（像素）</label></p>
                        <input id="xghome_watermark_text_px" type="number" min="12" max="72" step="1" name="xghome_watermark_text_px" value="<?php echo esc_attr((string) $options['watermark_text_px']); ?>" class="small-text">
                        <span class="description">仅作用于文字水印。默认 24，范围 12～72。</span>
                        <p><label for="xghome_watermark_skip_max_px">小图不加水印（方案 C）</label></p>
                        <input id="xghome_watermark_skip_max_px" type="number" min="16" max="512" step="1" name="xghome_watermark_skip_max_px" value="<?php echo esc_attr((string) $options['watermark_skip_max_px']); ?>" class="small-text">
                        <span class="description">正文内图片的宽、高（取较大边）≤ 此像素时不叠水印（与「表情路径 / xghome-emoji」排除叠加）。默认 64，范围 16～512。</span>
                    </td>
                </tr>
                <?php xghome_ip_geo_render_admin_fields($options); ?>
                <tr>
...
function xghome_classic_get_emoji_sets(): array
{
    ...
    foreach ($dirs as $dir) {
        ...
        foreach (['png', 'jpg', 'jpeg', 'gif', 'webp', 'svg'] as $ext) {
            foreach ([$ext, strtoupper($ext)] as $variant) {
                $found = glob($dir . '/*.' . $variant);
...
/**
 * 评论保存等：data、comment、pre_comment_content 等上下文默认不含 img 时会剥掉表情图。
 */
function xghome_classic_kses_allow_emoji_img_tags($tags, $context)
{
    if (!is_array($tags)) {
        return $tags;
    }
    $ctx = (string) $context;
    if (!in_array($ctx, ['data', 'comment', 'pre_comment_content'], true)) {
        return $tags;
    }
    $tags['img'] = [
        'src'      => true,
...
add_filter('wp_kses_allowed_html', 'xghome_classic_kses_allow_emoji_img_tags', 100, 2);

/**
 * 表情图 HTML。GIF 使用 loading="eager"，避免 lazy 解码只出第一帧导致「不动」。
 */
function xghome_classic_build_emoji_img_tag(string $url, string $alt_name): string
{
    $path = (string) wp_parse_url($url, PHP_URL_PATH);
    $is_gif = $path !== '' && (bool) preg_match('/\.gif$/i', $path);
    $loading = $is_gif ? 'eager' : 'lazy';

    return '<img class="xghome-emoji" src="' . esc_url($url) . '" alt="' . esc_attr($alt_name) . '" width="36" height="36" loading="' . $loading . '" decoding="async">';
}

/**
 * 正文/评论输出末尾：主题目录下的 .gif 表情强制 eager，覆盖核心注入的 lazy。
 */
function xghome_classic_fix_gif_emoji_img_tags(string $content): string
{
    if ($content === '' || stripos($content, 'images/emoji') === false || stripos($content, '.gif') === false) {
        return $content;
    }
    ...
    $fixed = preg_replace_callback(
        ...
        static function (array $m): string {
            $tag = $m[0];
            if (stripos($tag, 'images/emoji') === false) {
                return $tag;
            }
            if (!preg_match('/\.gif(\?|"|\'|\s|>)/i', $tag)) {
                return $tag;
            }
            $tag = preg_replace('/\sloading\s*=\s*(["\'])lazy\1/i', '', $tag);
            if (preg_match('/\sloading\s*=/i', $tag)) {
                $tag = preg_replace('/\sloading\s*=\s*["\'][^"\']*["\']/i', ' loading="eager"', $tag);
            } else {
...
add_filter('the_content', 'xghome_classic_fix_gif_emoji_img_tags', 999);
add_filter('comment_text', 'xghome_classic_fix_gif_emoji_img_tags', 999);
        ...
        <?php foreach ($emoji_sets as $index => $set) : ?>
            <div class="emoji-set-panel<?php echo $index === 0 ? ' is-active' : ''; ?>" data-emoji-panel="<?php echo esc_attr((string) $index); ?>">
                <?php foreach ($set['items'] as $emoji) :
                    $emoji_tag = xghome_classic_build_emoji_img_tag($emoji['url'], $emoji['name']);
                    ?>
                    <button type="button" class="emoji-item-btn" data-emoji-insert="<?php echo esc_attr($emoji_tag); ?>">
                        <?php echo wp_kses_post($emoji_tag); ?>
                    </button>
                <?php endforeach; ?>
            </div>
        <?php endforeach; ?>
...
function xghome_classic_print_media_style_vars(): void
{
    ...
    $watermark_text_px = max(12, min(72, (int) get_option('xghome_watermark_text_px', 24)));
    echo '<style>:root{--xghome-media-max-width:' . esc_html((string) $media_width) . '%;--xghome-image-max-height:' . esc_html((string) $image_height) . 'px;--xghome-video-max-height:' . esc_html((string) $video_height) . 'px;--xghome-audio-max-width:' . esc_html((string) $audio_width) . '%;--xghome-left-avatar-size:' . esc_html((string) $avatar_px) . 'px;--xghome-watermark-text-px:' . esc_html((string) $watermark_text_px) . 'px;}</style>';
}
...
function xghome_classic_print_watermark_vars(): void
{
    ...
    $skip_max = max(16, min(512, (int) get_option('xghome_watermark_skip_max_px', 64)));
    echo '<script>window.XGHOME_WATERMARK=' . wp_json_encode(['image' => $img, 'text' => $text, 'skipMaxPx' => $skip_max]) . ';</script>';
}
...
function xghome_classic_markdown_to_html(string $markdown): string
{
    ...
    $markdown = preg_replace_callback(
        '/<img\b[^>]*\bsrc\s*=\s*("|\')([^"\']*images\/emoji[^"\']*)\1[^>]*>/i',
        function ($m) use (&$emoji_imgs) {
            $full = (string) ($m[0] ?? '');
            $token = '__XGHOME_EIMG_' . count($emoji_imgs) . '__';
            $emoji_imgs[$token] = $full;
    ...
    foreach ($emoji_imgs as $token => $img_html) {
        $output = str_replace(htmlspecialchars($token, ENT_QUOTES, 'UTF-8'), $img_html, $output);
    }
...
function xghome_classic_render_post_content(int $post_id): void
{
    if (xghome_classic_markdown_enabled($post_id)) {
        ...
        echo wp_kses_post(xghome_classic_fix_gif_emoji_img_tags(xghome_classic_markdown_to_html($raw)));
        return;
    }
    the_content();
}
