(function () {
    ...
    document.addEventListener('click', function (e) {
        var el = e.target.closest('#comments .xghome-comment-replies-toggle');
        if (!el) {
            return;
        }
        e.preventDefault();
        var panel = el.nextElementSibling;
        if (!panel || !panel.classList.contains('xghome-comment-replies-panel')) {
            return;
        }
...
(function ($) {
    ...
    $(function () {
        ...
        if (wm && (wm.image || wm.text)) {
            var skipMax = typeof wm.skipMaxPx === 'number' && wm.skipMaxPx > 0 ? wm.skipMaxPx : 0;
            var shouldSkipWatermark = function ($img) {
                if ($img.hasClass('xghome-emoji')) {
                    return true;
                }
                var src = ($img.attr('src') || '').toLowerCase();
                if (src.indexOf('images/emoji') !== -1) {
                    return true;
                }
                if (!skipMax) {
                    return false;
                ...
                var rw = Math.round($img.outerWidth());
                var rh = Math.round($img.outerHeight());
                if (rw > 0 && rh > 0 && Math.max(rw, rh) <= skipMax) {
                    return true;
                }
                return false;
            };
