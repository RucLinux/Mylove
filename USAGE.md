...
> **分发版说明：** 本主题包为可公开发布版本：不包含任何特定站点的备案号、统计 ID、作者官网等硬编码。页脚备案与「网站统计」链接请在后台 **备案管理**、**Mylove 设置**（页脚统计 URL）中自行填写；百度/Google 等统计脚本请用 **统计代码**。若你曾在本仓库中做过「个人站」定制，完整备份副本在同级的 **`myzhenai-backup-personal`** 目录（仅本地备份，勿随主题分发）。
...
|------|------|
| 主题名称 | Mylove |
| 主题作者 | Mylove（见 `style.css` 主题头；可自行在子主题中改写） |
| 主题主页占位 | [WordPress.org Themes](https://wordpress.org/themes/)（发布到目录后可改为实际地址） |

许可：`GPL-2.0-or-later`（与 `style.css` 声明一致）。
...
| 菜单项 | 作用 |
|--------|------|
| **Mylove 设置** | 伪静态与文章链接格式、分类/标签/页面 `.html`、顶栏「统计 / 时光机」开关、**页脚「网站统计」链接 URL**（留空则不显示）、二维码 API（默认留空）、评论联系方式正则、媒体尺寸、视频解析 API、水印、默认缩略图、赞赏、头像源、防采集、**左右栏布局互换**、左侧头像尺寸等。保存后会按设置刷新重写规则。 |
| **备案管理** | 维护 ICP / 公安备案等 HTML，支持默认内容与**按访问域名**的规则。页脚仅在填写后显示对应区块。 |
...
- **文末展示：** 可显示最后修改时间等（无访客 IP / 地理展示）。
- **纪念日灰度：** 每年 **9 月 18 日**、**12 月 13 日** 全站 `html` 灰度滤镜（无节日灯笼等装饰）。
